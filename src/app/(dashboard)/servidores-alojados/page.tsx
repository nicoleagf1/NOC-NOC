...
import { Server, Shield, Network, Loader2, Edit2, X, AlertCircle, Plus, Eye, EyeOff, Lock, Copy, CheckCircle2 } from "lucide-react";
...
export default function ServidoresAlojadosPage() {
  ...
  const openHostFormModal = (host?: any) => {
    if (host) {
      setNewHost({
        ...host,
        asset_type: host.asset_type || 'HOST',
  ...
  const handleSaveHost = async () => {
    ...
    try {
      ...
      const isService = newHost.asset_type === 'SERVICE';
      
      const url = isService 
        ? (isEdit ? `/api/services/${newHost.id}` : '/api/services')
        : (isEdit ? `/api/hosts/${newHost.id}` : '/api/hosts');
        
      const method = isEdit ? 'PUT' : 'POST';

      let payload = { ...newHost };
      if (isService) {
        payload.name = newHost.hostname;
        payload.endpoint_url = newHost.ip_address;
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50">
              <tr className="text-xs font-bold text-gray-500 uppercase tracking-widest border-b border-gray-100">
                <th className="px-6 py-4">Activo / Endpoint</th>
                <th className="px-6 py-4">Entorno y Monitoreo</th>
                <th className="px-6 py-4">Tipo y Detalles</th>
                <th className="px-6 py-4 w-1/3">Relaciones (Hosts/Servicios)</th>
                ...
                <tr key={host.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="flex items-center cursor-pointer hover:opacity-80 transition-opacity" onClick={() => setViewingHost(host)}>
                      <div className={`w-12 h-12 rounded-lg ${host.asset_type === 'SERVICE' ? 'bg-emerald-50 border-emerald-100' : 'bg-indigo-50 border-indigo-100'} flex items-center justify-center mr-4`}>
                        {host.asset_type === 'SERVICE' ? <Network className="w-6 h-6 text-emerald-500" /> : <Server className="w-6 h-6 text-indigo-500" />}
                      </div>
                      <div>
                        <div className="font-bold text-vepagos-navy text-lg flex items-center hover:underline">
                          {host.hostname}
                          {host.vault_password && <span title="Credenciales Vault Asignadas"><Lock className="w-4 h-4 text-vepagos-green ml-2" /></span>}
                        </div>
                        <div className="text-sm text-gray-500 font-mono mt-0.5 max-w-[250px] truncate">{host.ip_address}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-2 items-start">
                      <Badge variant={host.environment === 'PROD' ? 'default' : 'info'} className="text-xs uppercase font-bold tracking-widest px-2 py-1">
                        {host.environment}
                      </Badge>
                      {host.is_monitored ? (
                        <div className="flex items-center text-xs font-bold text-emerald-600 bg-emerald-50 px-2 py-1 rounded-md border border-emerald-100">
                          <span className="w-2 h-2 rounded-full bg-emerald-500 mr-1.5 animate-pulse"></span>
                          Monitor Activo
                        </div>
                      ) : (
                        <div className="flex items-center text-xs font-bold text-gray-500 bg-gray-100 px-2 py-1 rounded-md border border-gray-200">
                  ...
                  <td className="px-6 py-4">
                    <div className="font-bold text-gray-700 text-base">{host.asset_type === 'SERVICE' ? 'Servicio Web' : host.os_type}</div>
                    <div className="text-xs text-gray-500 uppercase tracking-widest font-bold mt-1">Rol: {host.server_role}</div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-2">
                      {host.services.length === 0 ? (
                        ...
                        host.services.map((svc: any) => (
                          <Badge key={svc.id} variant="info" className="text-xs bg-white border-gray-200 text-gray-600 px-2 py-1">
                            {svc.name}
                          </Badge>
                        ))
                      )}
                    ...
                    <div className="flex justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      {host.asset_type === 'HOST' && (
                        <Button 
                          variant="outline" 
                          size="sm" 
                          className="h-10 text-xs font-bold text-vepagos-green hover:bg-vepagos-green/10 border-vepagos-green px-3"
                          onClick={() => openHostModal(host)}
                        >
                          <Network className="w-4 h-4 mr-2" /> Mapeo
                        </Button>
                      )}
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        className="h-10 w-10 p-0 text-gray-500 hover:text-vepagos-navy"
                      ...
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        className="h-10 w-10 p-0 text-vepagos-navy hover:text-vepagos-navy"
          ...
          <Card className="w-full max-w-3xl bg-white overflow-hidden shadow-2xl flex flex-col rounded-xl border-0 relative my-auto">
            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6 overflow-y-visible">
              ...
              <div className="space-y-4">
                <h3 className="text-[11px] font-bold text-gray-500 uppercase tracking-widest border-b pb-2">1. Detalles del Activo</h3>
                
                <div className="flex items-center space-x-4 mb-4">
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" name="asset_type" value="HOST" checked={newHost.asset_type === 'HOST'} onChange={() => setNewHost({...newHost, asset_type: 'HOST'})} className="text-vepagos-green focus:ring-vepagos-green" disabled={!!newHost.id} />
                    <span className="text-sm font-bold text-gray-700">Servidor / Host</span>
                  </label>
                  <label className="flex items-center space-x-2 cursor-pointer">
                    <input type="radio" name="asset_type" value="SERVICE" checked={newHost.asset_type === 'SERVICE'} onChange={() => setNewHost({...newHost, asset_type: 'SERVICE'})} className="text-vepagos-green focus:ring-vepagos-green" disabled={!!newHost.id} />
                    <span className="text-sm font-bold text-gray-700">Servicio Web</span>
                  </label>
                </div>

                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{newHost.asset_type === 'HOST' ? 'Nombre (Hostname)' : 'Nombre del Servicio'}</label>
                  <input type="text" className="w-full text-sm border border-gray-200 rounded-md p-2 bg-gray-50 focus:bg-white" value={newHost.hostname} onChange={e => setNewHost({...newHost, hostname: e.target.value})} placeholder={newHost.asset_type === 'HOST' ? "Ej: srv-db-01" : "Ej: API Gateway"} />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{newHost.asset_type === 'HOST' ? 'Dirección IP' : 'URL / Endpoint'}</label>
                  <input type="text" className="w-full text-sm border border-gray-200 rounded-md p-2 bg-gray-50 focus:bg-white" value={newHost.ip_address} onChange={e => setNewHost({...newHost, ip_address: e.target.value})} placeholder={newHost.asset_type === 'HOST' ? "Ej: 192.168.1.10" : "Ej: https://api.ejemplo.com"} />
                </div>
                
                {newHost.asset_type === 'HOST' && (
                  <>
                  ...
                  <div>
                    ...
                    <select className="w-full text-sm border border-gray-200 rounded-md p-2 bg-gray-50" value={newHost.environment || 'PROD'} onChange={e => setNewHost({...newHost, environment: e.target.value})}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    ...
                    <input type="text" className="w-full text-sm border border-gray-200 rounded-md p-2 bg-gray-50 focus:bg-white" value={newHost.server_role} onChange={e => setNewHost({...newHost, server_role: e.target.value})} placeholder="Ej: Base de Datos" />
                  </div>
                </div>
                  </>
                )}
                ...
                <div>
                  ...
                  <textarea className="w-full text-sm border border-gray-200 rounded-md p-2 bg-gray-50 focus:bg-white" rows={2} value={newHost.description || ''} onChange={e => setNewHost({...newHost, description: e.target.value})} placeholder={`Breve descripción del ${newHost.asset_type === 'HOST' ? 'servidor' : 'servicio'}...`} />
                </div>
                
                <div className="flex items-center mt-2 p-3 border border-indigo-100 bg-indigo-50/50 rounded-md">
                  <input type="checkbox" id="is_monitored_vault" checked={newHost.is_monitored} onChange={e => setNewHost({...newHost, is_monitored: e.target.checked})} className="mr-2 text-vepagos-green focus:ring-vepagos-green rounded" />
                  <label htmlFor="is_monitored_vault" className="text-[11px] font-bold text-vepagos-navy cursor-pointer">Monitorear activamente este activo</label>
                </div>
              ...
              <div className="space-y-4">
                ...
                {/* Crear nuevo servicio inline */}
                <div className="flex items-center gap-2 mb-3 bg-white p-2 rounded-md border border-gray-200">
                  <input 
                    type="text" 
                    placeholder="Crear un servicio nuevo..." 
                    className="flex-1 text-sm bg-transparent border-0 outline-none px-2"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <Card className="w-full max-w-md bg-white overflow-hidden shadow-2xl flex flex-col max-h-[90vh] rounded-xl border-0">
            <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50/50">
              ...
              <button onClick={() => setViewingHost(null)} className="text-gray-400 hover:text-gray-600 bg-white p-2 rounded-full border border-gray-200 hover:bg-gray-100 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            
            <div className="p-6 space-y-4 overflow-y-auto bg-gray-50/30">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{viewingHost.asset_type === 'SERVICE' ? 'Nombre' : 'Hostname'}</label>
                  <div className="text-sm font-bold text-vepagos-navy">{viewingHost.hostname}</div>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{viewingHost.asset_type === 'SERVICE' ? 'Endpoint / URL' : 'Dirección IP'}</label>
                  <div className="text-sm font-mono text-gray-700 truncate" title={viewingHost.ip_address}>{viewingHost.ip_address}</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Tipo y Detalles</label>
                  <div className="text-sm text-gray-700">{viewingHost.asset_type === 'SERVICE' ? 'Servicio Web' : viewingHost.os_type} - {viewingHost.server_role}</div>
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Descripción</label>
                <div className="text-sm text-gray-700 bg-white p-3 rounded-md min-h-[50px] border border-gray-200">
                  {viewingHost.description || <span className="text-gray-400 italic">Sin descripción asignada.</span>}
                </div>
              </div>

              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Seguridad (Vault)</label>
                {viewingHost.vault_username ? (
                  <div className="bg-white border border-gray-200 rounded-md overflow-hidden">
                    <div className="flex items-center justify-between p-2.5 border-b border-gray-100 bg-gray-50/50">
                      <div className="flex items-center text-sm text-gray-600">
                        <Lock className="w-4 h-4 text-vepagos-green mr-2" />
                        Usuario
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="font-mono font-bold text-sm text-vepagos-navy">{viewingHost.vault_username}</span>
                        <button onClick={() => handleCopy(viewingHost.vault_username, 'username')} className="text-gray-400 hover:text-vepagos-navy transition-colors" title="Copiar Usuario">
                          {copiedField === 'username' ? <CheckCircle2 className="w-4 h-4 text-green-500" /> : <Copy className="w-4 h-4" />}
                        </button>
                      </div>
                    </div>
                    {viewingHost.vault_password && viewingHost.vault_password !== 'ERROR_DECRYPTING' && (
                      <div className="flex items-center justify-between p-2.5 bg-white">
                        <div className="flex items-center text-sm text-gray-600">
                          <Lock className="w-4 h-4 text-vepagos-green mr-2 opacity-0" />
                          Contraseña
                        </div>
                        <div className="flex items-center gap-3">
                          <span className="font-mono text-sm text-vepagos-navy tracking-widest">
                            {showViewPassword ? viewingHost.vault_password : '••••••••••••'}
                          </span>
                          <div className="flex items-center gap-1 border-l border-gray-200 pl-3 ml-1">
                            <button onClick={() => setShowViewPassword(!showViewPassword)} className="text-gray-400 hover:text-vepagos-navy transition-colors p-1" title="Mostrar/Ocultar">
                              {showViewPassword ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                            </button>
                            <button onClick={() => handleCopy(viewingHost.vault_password, 'password')} className="text-gray-400 hover:text-vepagos-navy transition-colors p-1" title="Copiar Contraseña">
                  ...
                  <div className="text-sm text-gray-700 bg-white p-3 rounded-md border border-gray-200 flex items-center">
                    <AlertCircle className="w-4 h-4 text-gray-400 mr-2" /> <span className="text-gray-400 italic">Sin credenciales en el Vault</span>
                  </div>
                )}
              </div>

              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">{viewingHost.asset_type === 'SERVICE' ? 'Hosts Relacionados' : 'Servicios Alojados'}</label>
                <div className="flex flex-wrap gap-2 mt-1">
                  {viewingHost.services && viewingHost.services.length > 0 ? (
                    viewingHost.services.map((svc: any) => (
                      <Badge key={svc.id} variant="info" className="bg-white border-gray-200 text-vepagos-navy text-xs">
                        {svc.name}
                      </Badge>
                    ))
                  ) : (
            ...
            <div className="p-6 border-t border-gray-100 bg-white flex justify-end gap-3 rounded-b-xl">
              ...
              <Button 
                ...
                onClick={() => {
                  openHostFormModal(viewingHost);
                  setViewingHost(null);
                }}
              >
                <Edit2 className="w-4 h-4 mr-2" /> Editar Servidor
              </Button>
            </div>
          </Card>
        </div>
